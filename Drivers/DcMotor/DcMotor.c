...
int16_t FloatToInt16Limit(float f, int limit) 
{
  ...
  //소수점 이하 자리를 사사오입해서 소수점자리가 0인 실수형
   ...
   if( f1 >=0)      u= (int16_t) MIN(f1, U_MAX) ;// 30<=(30, 100), 100<=(150, 100)
    else   u = (int16_t) MAX (f1, -U_MAX) ; // -30<=(-30, -100), -100<=(-150, -100)
...
{
   int16_t result;
   if(u>0)  result = MAX(u, requirement_plus); //양의 PWM펄스 최대값 입력
